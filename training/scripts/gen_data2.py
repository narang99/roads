...
class RandomSampler:
    def __init__(self, images_bases: list[Path], frags_base: Path, yolo_cls_label_id: int = 0):
        self.images = list(images_base.rglob("*.jpg") for images_base in images_bases)
...
def run_sampler(images_bases, frags_base, num_samples, dest_folder):
    images_bases = list(map(Path, images_bases))
    frags_base = Path(frags_base)
    dest_folder = Path(dest_folder)

    sampler = RandomSampler(images_bases, frags_base)
...
if __name__ == "__main__":
    ...
    run_sampler([image_base], frags_base, num_samples, dest_folder)
