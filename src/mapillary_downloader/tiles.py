...
def get_tiles_for_bbox(
    min_lon: float, min_lat: float, max_lon: float, max_lat: float, zoom: int = 14
) -> list[Tile]:
    """
...
def tile_to_small_bboxes(
    tile: Tile, max_area_degrees: float = 0.01
    ...
    while lat < max_lat:
        ...
        while lon < max_lon:
            bboxes.append(
                (lon, lat, min(lon + step, max_lon), min(lat + step, max_lat))
            )
            lon += step
        lat += step
