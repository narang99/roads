...
async def fetch_image_metadata(
    ...
    if fields is None:
        fields = [
            ...
            "is_pano", "camera_type", "make", "model"
        ]
...
async def fetch_multiple_image_metadata(
    ...
    async def fetch_one(image_id: str) -> Optional[dict]:
        async with semaphore:
            try:
                return await fetch_image_metadata(client, access_token, image_id, fields)
...
async def download_image_with_metadata(
    client: httpx.AsyncClient,
    access_token: str,
    image_id: str,
    output_dir: Path,
    ...
    """
    Fetch image metadata and download the image.
    ...
    Args:
        ...
        size_field: Which thumbnail size to download
    
    Returns:
        Tuple of (success, local_path, metadata)
    """
    try:
        # Fetch metadata
        metadata = await fetch_image_metadata(
            client, access_token, image_id,
            fields=["id", "captured_at", "computed_geometry", "computed_compass_angle",
                    size_field, "is_pano", "camera_type"]
        )
        
        image_url = metadata.get(size_field)
        if not image_url:
            return False, None, metadata
        
        # Determine output path
        ext = ".jpg"  # Mapillary images are JPEGs
