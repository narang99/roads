...
T006_DIR = DS / "T006-morphed"
T007_DIR = DS / "T007-morph-before-resize"
EXP_BASE = T007_DIR / PROJECT_CODE
OUTS = EXP_BASE / "synth"
...
import cv2
from mtrain.utils import pipe
from mtrain.smallnet.tfms import PaddedResize


def gradient(img):
    return cv2.morphologyEx(
        img, cv2.MORPH_GRADIENT, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    )

tfms = pipe(gradient, PaddedResize(FILE_SIZE))
...
res = tile_image_and_predict(TEST_BIG_IMG, learner, FILE_SIZE, add_labels=False, tfms=tfms)
