...
DUAL_CROP_EVAL_TFMS = v2.Compose([
...
class DualCropDataset(torch.utils.data.Dataset):
    """
    Dataset for dual-crop classification using only tight and medium crops.
    
    Creates 8-channel input from 2 scales: tight crop and medium crop.
    ...
    def _load_crop_pairs(self, directory: Path) -> list[tuple[np.ndarray, np.ndarray]]:
        """Load the 2-level crop pairs (tight and medium only) using leveled cropping with optional source augmentation."""
        ...
        # Create 3-level crops then take only first two
        crop_pair = make_crop_level_pairs_v2(
            sample, self.tight_pad, self.medium_pad, self.medium_center_prob
        )
        # Return only tight and medium crops (first two pairs)
        return crop_pair.pairs[:2]
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            x: 8-channel tensor [tight_rgb, medium_rgb, tight_mask, medium_mask]
            y: Label tensor (class index)
        """
        directory = self.dirs[index]
        
        # Load 2-level crop pairs (tight and medium only)
        pairs = self._load_crop_pairs(directory)
        assert len(pairs) == 2, f"Expected 2 pairs, got {len(pairs)} from {directory}"
        ...
        # Stack: [tight_img, medium_img, tight_mask, medium_mask] = 8 channels
    ...
    @classmethod
    def denormalize(
        cls, combined: torch.Tensor, pairs_per_dir: int = 2
    ) -> list[tuple[np.ndarray, np.ndarray]]:
        """
        Denormalize 8-channel tensor back to list of (image, mask) pairs.
        
        Args:
            combined: 8-channel tensor [tight_img, medium_img, tight_mask, medium_mask]
            pairs_per_dir: Number of pairs (always 2 for dual crop)
    ...
    def num_channels(self) -> int:
        """Number of input channels (always 8 for dual crop)."""
        return 8


def create_dual_crop_dataloaders(
    ...
    **dataset_kwargs
    ...
    Args:
        ...
        **dataset_kwargs: Additional arguments for DualCropDataset
