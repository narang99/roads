...
class BboxAnnotation:
    ...
    def is_hard_example(self) -> bool:
        ...
        return self.is_hard or (self.human_label is not None and self.human_label != self.original_pred)
...
class MassAnnotationWidget:
    ...
    def __init__(self, output_dir: str | Path, learner, crop_pad: int = 220, theme: str = "Dark"):
    ...
    def _reconstruct_probability_masks(self, all_probs: np.ndarray):
        ...
        for i, bbox in enumerate(self._bboxes):
            ...
            self._other_prob_mask[bbox.y:bbox.y2, bbox.x:bbox.x2][bbox_mask] = other_prob
            self._trash_prob_mask[bbox.y:bbox.y2, bbox.x:bbox.x2][bbox_mask] = trash_prob
    ...
    def _build_ui(self):
        ...
        # Set backgrounds
        self._out_main_image.layout.background = self._bg
        self._out_tight_crop.layout.background = self._bg
        self._out_medium_crop.layout.background = self._bg
        
        # Layout assembly
        overlay_controls = widgets.HBox([
            self._trash_overlay_toggle,
            self._other_overlay_toggle
    ...
    def _render_main_image(self):
        ...
        if (self._show_trash_overlay or self._show_other_overlay) and self._trash_prob_mask is not None and self._other_prob_mask is not None:
    ...
    def _create_prediction_overlay(self) -> np.ndarray:
        ...
        if self._show_trash_overlay and self._show_other_overlay:
            ...
            trash_wins = (self._trash_prob_mask >= self._other_prob_mask) & has_prediction
            other_wins = (self._other_prob_mask > self._trash_prob_mask) & has_prediction
            ...
            trash_regions = (self._trash_prob_mask >= self._other_prob_mask) & has_prediction
            overlay[trash_regions] = (255, 0, 0)  # Red for trash
        elif self._show_other_overlay:
            # Show only other predictions
            other_regions = (self._other_prob_mask > self._trash_prob_mask) & has_prediction
            overlay[other_regions] = (0, 255, 0)  # Green for other
        
        return overlay
    
    def _create_probability_overlay(self, prob_mask: np.ndarray, color: tuple, alpha: float = 0.4) -> np.ndarray:
    ...
    def _blend_overlay(self, base_image: np.ndarray, overlay: np.ndarray, alpha: float = 0.4) -> np.ndarray:
    ...
    def _update_selection_display(self):
        ...
        # Update status
        self._selection_status.value = f"Region {bbox_idx + 1}/{len(self._bboxes)} selected"
        pred_name = "Trash" if annotation.original_pred == LABEL_TRASH else "Other"
        self._model_status.value = f"Model: {pred_name} ({annotation.original_prob:.2f})"
    ...
    def _update_stats(self):
        ...
        annotated_count = sum(1 for ann in self._annotations.values() if ann.is_annotated)
    ...
    def _done_file(self) -> Path:
        return self._output_dir / "done_names.txt"
    ...
    def _save_hard_examples(self):
        ...
        for annotation in hard_annotations:
            try:
                ...
                saved_examples.append({
                    ...
                    'is_disagreement': annotation.human_label is not None and annotation.human_label != annotation.original_pred
                })
                saved_count += 1
        ...
        # Save summary files
        self._save_summary_files(saved_examples)
        ...
        print(f"Successfully saved {saved_count} hard examples for image '{self._name}'")
    
    def _save_single_crop(self, annotation: BboxAnnotation) -> Path:
        """Save a single crop following widget_2.py format."""
        bbox = annotation.bbox
        final_label = annotation.final_label
        folder = _LABEL_FOLDER[final_label]
        
        # Create sample directory
        sample_dir = self._output_dir / "crop_level" / folder / f"{self._name}_{annotation.bbox_idx}"
        sample_dir.mkdir(parents=True, exist_ok=True)
        
        # Create padded crop and mask
        ...
        # Create metadata
        meta = {
            "crop_origin": {"x": int(x1c), "y": int(y1c)},
            "model_pred": {
                "label": int(annotation.original_pred),
                "prob": float(annotation.original_prob)
            },
            "hard_example": True,
            "manual_annotation": annotation.human_label is not None
        }
        
        if annotation.human_label is not None and annotation.human_label != annotation.original_pred:
        ...
        # Create source directory symlink
    ...
    def _save_summary_files(self, saved_examples: List[dict]):
        """Save summary files for tracking."""
        # Save hard examples list
        hard_examples_file = self._output_dir / "hard_examples_list.txt"
        with hard_examples_file.open("a") as f:
            for example in saved_examples:
                f.write(f"{self._name}_{example['bbox_idx']}\n")
        ...
        summary_file = self._output_dir / "annotation_summary.json"
        summaries = []
...
# ------------------------------------------------------------------
# Utility functions (similar to widget_2.py)
# ------------------------------------------------------------------

def get_crops_for_image(image: np.ndarray, mask: np.ndarray, bbox_pad=20, crop_pad=220):
    """Get bounding boxes for connected components in the mask."""
    bboxes = list(get_region_crops(image, mask, bbox_pad))
    yield from iter_crops(image, mask, bboxes, crop_pad)


def iter_crops(image: np.ndarray, mask: np.ndarray, bboxes: List[Bbox], pad: int = 220):
    """Yield (bbox, crop_img, crop_mask) for each bbox."""
    for bbox in bboxes:
...
def create_mass_annotation_widget(output_dir: str | Path, learner, **kwargs) -> MassAnnotationWidget:
    """Create a new MassAnnotationWidget instance."""
    return MassAnnotationWidget(output_dir, learner, **kwargs)
    ...
    for label_folder in crop_level_dir.iterdir():
        ...
        for sample_dir in label_folder.iterdir():
            ...
            try:
                ...
                model_pred_label = metadata.get("model_pred", {}).get("label", "unknown")
                ...
                print(f"  - Model Prediction: {model_pred_label} (prob: {model_pred_prob:.3f})")
...
def process_image_directory(image_dir: Path, mask_dir: Path, output_dir: Path, learner):
    """
    Example function for processing multiple images.
    
    Usage:
        widget = create_mass_annotation_widget("/path/to/output", learner)
        ...
        widget.ui(source_dir, bboxes, image, mask)
        # User interacts with widget
        # User clicks "Save Hard Examples" when done
    """
