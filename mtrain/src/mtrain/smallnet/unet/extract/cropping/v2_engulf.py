...
def skewed_coords(
    ...
    # we want to force them to be towards max left and max right directly

    pad_left = add_jitter_pixels(max_left)
    pad_right = add_jitter_pixels(max_right)
    pad_left, pad_right = _scale_paddings(pad_left, pad_right, min_length)
    pad_top = add_jitter_pixels(max_top)
    pad_bottom = add_jitter_pixels(max_bottom)
