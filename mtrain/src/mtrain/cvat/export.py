...
def _encode_image_annotations(
    ...
    for mask_val in np.unique(mask_array):
        ...
        for comp in split_connected_components(binary_mask, min_area):
...
def _build_task_json(
    ...
    if job_size is None:
        jobs = [{"status": "annotation", "type": "annotation", "start_frame": 0, "stop_frame": stop_frame}]
    else:
        jobs = []
        start = 0
        while start <= stop_frame:
            end = min(start + job_size - 1, stop_frame)
            jobs.append({"status": "annotation", "type": "annotation", "start_frame": start, "stop_frame": end})
            start = end + 1
...
def export_cvat_annotations(
    ...
    for img_id, (image_name, _, mask_array) in enumerate(
        ...
        new_anns, ann_id = _encode_image_annotations(
            mask_array, img_id, ann_id, label_map, value_to_cat_id, image_name, min_area
        )
...
def export_cvat_task_backup(
    ...
    with tempfile.TemporaryDirectory(prefix="cvat_backup_") as tmp:
        ...
        with (staging_dir / "task.json").open("w") as f:
            json.dump(
                _build_task_json(
                    task_name, task_labels, stop_frame, image_quality, chunk_size, job_size
                ),
                f,
                indent=4,
            )
