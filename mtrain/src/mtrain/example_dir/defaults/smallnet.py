...
def default_smallnet_learners(models_dir, labels, bs) -> dict[str, SmallnetLearner]:
    ...
    if "sm" in labels:
        res["sm"] = get_smallnet_sm(models_dir, bs, data_dir)
    return res
...
def get_smallnet_v1(models_dir, bs):
    ...
    learner.eval()
    return SmallnetLearner("v1", learner, bs, 100, [50], 2, None)
...
def get_smallnet_md(models_dir, bs, data_dir):
    ...
    return SmallnetLearner("md", learner, bs, 128, [64], 35, None)
...
def get_smallnet_sm(models_dir, bs, data_dir):
    ...
    return SmallnetLearner("sm", learner, bs, 64, [32], None, 1500)