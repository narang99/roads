...
This point, any model I train basically increases the number of detections at smaller image sizes. The problem right now is that it over‑detects and also misses some of the cartridge pieces. It over-detects, even flagging simple cracks in the ground as garbage. At this point, I might try over-detecting and then let a model decide whether something is garbage or not.  
